...
function buildSeamlessLoop(items, spacing) {
	...
	for (i = 0; i < l; i++) {
		...
		item = items[index];
		time = i * spacing;
		rawSequence.fromTo(item, {scale: 0, opacity: 0}, {scale: 1.4, opacity: 1, zIndex: 100,className: "selected", duration: 0.5, yoyo: true, repeat: 1, ease: "power1.in", immediateRender: false}, time)
...
$("li").on("click",function(event){
	...
	let opacityValue = $(this).css('opacity');
	if (opacityValue == '1') {
		$(this).closest('ul').addClass("large-it")
		$(this).addClass("large-it")
	}
})


$('body').on('click', function(e) {
	if (!$(e.target).hasClass('my-image') ) {
	  $('.large-it').removeClass("large-it")
	}
  });
